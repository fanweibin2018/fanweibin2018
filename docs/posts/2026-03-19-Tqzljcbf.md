...
新版 OpenClaw 不再读取系统的 `http_proxy` 环境变量，需要通过自身配置项单独设置代理，否则无法访问 Claude / OpenAI 等国外模型 API：

```
openclaw config set proxy.enabled true
openclaw config set proxy.proxyUrl http://127.0.0.1:7890
openclaw gateway restart
```

> - Mihomo/Clash 跑在本机时，`proxyUrl` 填 `http://127.0.0.1:7890`；若代理在局域网另一台机器，改成 `http://<代理机IP>:7890`。
> - 验证：`openclaw config get proxy`，确认 `enabled` 为 `true`、`proxyUrl` 正确。
...
| OpenClaw 代理 | 新版必须 `openclaw config set proxy.enabled true` + `proxy.proxyUrl`，否则无法访问国外模型 API |
