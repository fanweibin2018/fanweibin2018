...
### 8.4 OpenClaw 代理（访问国外 AI 模型必需）

新版 OpenClaw 不读取系统的 `http_proxy` 环境变量，需要通过自身配置项单独设置代理，否则无法访问 Claude / OpenAI 等国外模型 API：
...
> 代理在本机时填 `http://127.0.0.1:7890`；在局域网另一台机器则填 `http://<代理机IP>:7890`。验证：`openclaw config get proxy`。
...
| OpenClaw | `openclaw config set proxy.enabled true` + `proxy.proxyUrl` | 不读环境变量，改完需 `gateway restart` |
...
5. **代理配置各自独立**：Shell 环境变量 / Docker Daemon / npm / OpenClaw 等都要分别配置，没有统一入口；OpenClaw 还需用 `proxy.enabled` + `proxy.proxyUrl` 单独设置才能访问国外模型
