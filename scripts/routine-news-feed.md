...
- WebFetch 失败(403 / 404 / 超时)→ 丢弃该条目,**禁止凭记忆/凭搜索摘要补全**

#### 步骤 1 —— 标题校验(防"URL 对的、标题瞎编的")

抓回 HTML 后,从下列任一处取出页面的"真实标题":
- `<title>` 标签去掉站点后缀(如 ` - IT之家`)
...
举例(应丢弃 / 覆盖):
- URL = `ithome.com/0/947/947.htm`,页面真实标题是「消息称 DeepSeek 寻求 500 亿元融资,下月将发布 V4.1 更新」
- 你不能写成「消息称 DeepSeek V4.1 定档 6 月,新增原生多模态与 MCP 适配」—— **这是伪造,必须丢弃或改回真实标题**
