...
### ⚠️ `publishedAt` 时间提取规则(硬性,违反则丢弃该条目)

**绝对禁止伪造时间。看不到精确时间就只写日期,不要给一个 `10:00:00` / `08:00:00` / `00:00:00` 这种整点占位值。**
...
4. **页面正文里的时间标记**
   - 中文站点常见:`2026-05-13 08:30`、`2026年5月13日 08:30`、`05-13 08:30` (今年补全)
...
**自检规则**(回报前 routine 自己跑一遍):
- 同一批 items 里出现 ≥3 条 `publishedAt` 时间字段都是 `10:00:00` 或 `08:00:00` → 强烈怀疑伪造,**整批回退**到 date-only 形式
- `publishedAt` 不得晚于 routine 当前运行时间(未来时间一律丢弃或退化为 date-only)
